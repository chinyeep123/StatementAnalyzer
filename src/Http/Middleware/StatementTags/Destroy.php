...
/** --------------------------------------------------------------------------------
 * This middleware class handles [destroy] precheck processes for statement_tags
 *
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class Destroy {
    ...
    public function handle($request, Closure $next) {
        ...
        if (auth()->user()->is_team) {
            if (auth()->user()->role->role_statement_tags >= 3) {
                return $next($request);
            }else{
                abort(403);
            }
        ...
        Log::error("no items were sent with this request", ['process' => '[permissions][statement_tags][destroy]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'statement id' => $statement_id ?? '']);
        abort(409);
    }
}
